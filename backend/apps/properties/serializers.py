...
class PropertyCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_price_kes(self, value):
        if value < 10000:
            raise serializers.ValidationError("Price must be at least KES 10,000.")
        return value
...
class PropertyDetailSerializer(serializers.ModelSerializer):
    """Full detail view including seller info and verification data."""
    price         = serializers.SerializerMethodField()
    size          = serializers.SerializerMethodField()
    last_verified = serializers.SerializerMethodField()
    ...
    class Meta:
        model  = Property
        fields = [
            "id", "title", "description", "county", "sub_county", "area_name",
            "latitude", "longitude", "lr_number", "property_type",
            "size", "price", "price_negotiable", "status",
            "is_verified_pro", "trust_score", "last_verified",
            "seller_name", "created_at",
        ]
        # Never expose lr_number_encrypted in API responses
        read_only_fields = fields

    def get_price(self, obj):         return f"KES {obj.price_kes:,}"
    def get_size(self, obj):          return f"{obj.size_acres} Acres"
    def get_seller_name(self, obj):   return obj.seller.get_full_name()
    def get_last_verified(self, obj):
        if not obj.last_verified_at:
            return "Not yet verified"
        from django.utils.timesince import timesince
